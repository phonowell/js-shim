...
__change_index_jsShim__ := Func("jsShim_53")
global $ := {}
$.beep := Func("jsShim_52")
...
jsShim_17(index, callback) {
  callback.Call()
  __ts_throttle__[__change_index_jsShim__.Call(index)] := $.now.Call()
}
jsShim_18(index, time, callback) {
  clearTimeout.Call(__timer_throttle__[__change_index_jsShim__.Call(index)])
  __timer_throttle__[__change_index_jsShim__.Call(index)] := setTimeout.Call(Func("jsShim_17").Bind(index, callback), __ts_throttle__[__change_index_jsShim__.Call(index)] - $.now.Call() + time)
}
jsShim_19(time, callback) {
  __index_throttle__++
  __ts_throttle__[__change_index_jsShim__.Call(__index_throttle__)] := 0
...
jsShim_24(input) {
  ...
  for i, key in input {
    i := i - 1
    __output__[__change_index_jsShim__.Call(__len__ - i + 1)] := key
  }
  return __output__
}
...
jsShim_29(listInput*) {
  ...
  for i, key in __listKey__ {
    i := i - 1
    if (i == __len__) {
      __listResult__[__change_index_jsShim__.Call(i)] := $.split.Call(key, ":")
      continue
    }
    if ($.includes.Call(key, ":")) {
      __listResult__[__change_index_jsShim__.Call(i)] := $.split.Call(key, ":")
      __listResult__[__change_index_jsShim__.Call(__len__ * 2 - i)] := $.split.Call(key, ":")
    } else {
      __listResult__[__change_index_jsShim__.Call(i)] := [key, "down"]
      __listResult__[__change_index_jsShim__.Call(__len__ * 2 - i)] := [key, "up"]
    }
  }
  for i, it in __listResult__ {
    ...
    __listResult__[__change_index_jsShim__.Call(i)] := $.trim.Call("" . (it[2]) . " " . (it[3]) . "")
  }
  __output__ := ""
...
jsShim_48(index, time, callback) {
  clearTimeout.Call(__timer_debounce__[__change_index_jsShim__.Call(index)])
  __timer_debounce__[__change_index_jsShim__.Call(index)] := setTimeout.Call(callback, time)
}
